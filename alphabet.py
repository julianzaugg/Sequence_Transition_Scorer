...
def convertSeqs(seqs, new_alpha):
    alpha = Alphabet(''.join(new_alpha.values()))
    for seq in seqs:
        new_sequence = list(seq.sequence)
        for c in range(len(new_sequence)):
            new_sequence[c] = new_alpha.get(new_sequence[c], new_sequence[c])
        seq.sequence = ''.join(new_sequence)
        seq.alphabet = alpha
    return seqs
...
Alpha10 = Alphabet(sorted(list(set(alpha_10.values()))))
